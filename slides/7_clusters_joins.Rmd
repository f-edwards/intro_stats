...
```{r}
ggplot(afghan,
       aes(y = age, x = educ.years, color = province)) + 
...
ggplot(afghan,
       aes(y = age, x = educ.years)) + 
  geom_jitter(alpha = 0.3, aes(color = province)) + 
  geom_smooth(method = "lm")
```
...
```{r size = "tiny"}
results<-read_csv("https://raw.githubusercontent.com/f-edwards/intro_stats/master/data/1976-2016-president.csv")
head(results)
results<-results %>% 
  filter(year==2016) %>% 
