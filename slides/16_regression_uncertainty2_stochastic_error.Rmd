...
fake_data<-data.frame(hours_tv_per_school_day = 0:5)

e_y<-predict(m1, newdata = fake_data,
             interval = "confidence")

e_y2<-predict(m1, newdata = fake_data,
              interval = "prediction") %>% 
  data.frame() %>% 
  rename(fit2 = fit,
         lwr2 = lwr,
...
```{r}
m1<-lm(school_night_hours_sleep ~ 
         hours_tv_per_school_day,
       data = dat)

m2<-lm(school_night_hours_sleep ~ 
         hours_tv_per_school_day + 
         age,
       data = dat)

m3<-lm(school_night_hours_sleep ~ 
         hours_tv_per_school_day + 
         age + 
         height,
       data = dat)
...
m4<-lm(school_night_hours_sleep ~ 
         hours_tv_per_school_day + 
         age + 
         weight + 
