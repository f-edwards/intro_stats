...
We put a $\hat{hat}$ on variables to indicate that they are estimated from the data.
...
In other words, we solve for the values of $\beta_0$ and $\beta_1$ that result in the smallest possible value for SSR \pause

\[ \textrm{SSR} = \sum_{i=1}^n \varepsilon_i^2 =  \sum_{i=1}^n (y_i - \beta_0 - \beta_1 X )^2\] 

## Solving for Beta

We can solve for the values of $\beta$ that minimize the sum of squares using linear algebra (R does this for us).
...
Note that `~` separates the left and ride side of the model (where outcomes are on the left, predictors on the right). This is called formula notation in `R`
...
# Practice with estimation, interpretation on homework