...
::: {.incremental}
- Given the points $(1,3)$ and $(4,6)$, compute the slope and y intercept for a line that intersects both points. 
- the slope is $m = \frac{6-3}{4-1} = 1$ 
- the y-intercept is $b = 3 - 1 \cdot 1 = 2$. The equation of the line is $y= 1x + 5$
:::
...
**Compute slope, y-intercept, and sketch a line for each pair of coordinates**
...
Represented as $\sum$, with begin and end points 
...
$\mathbf{X}_{ij}$ refers to the value in the $i$th row and $j$th column of matrix $\mathbf{X}$.
