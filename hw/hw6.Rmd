...
Load the data using `library(qss)` and `data(vignettes)`
...
respondents seem to have a higher degree of
...
age variable separately for Mexican and Chinese respondents. Examine the mean value for each country. Examine the standard deviation for each country. What differences in age distribution do
...
or higher than `Jane` but lower than `Alison`, and 4 if ranked as high as `Alison` or higher.  Create barplots of this new
variable as in Question 1. Also,
