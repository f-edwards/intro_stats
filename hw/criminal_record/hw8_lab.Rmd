...
library(qss)
library(tidyverse)
cj<-read_csv("criminalrecord.csv")
```
...
```{r}
cj_q2<-cj %>% 
  group_by(crimrec, black) %>% 
  summarise(#callback = sum(callback),
            #total = n(),
            xbar = sum(callback)/n(),
            se = sd(callback)/sqrt(n()))

### formula: xbar +- se * crit value

crit<-qnorm(0.975,0,1)

cj_q2<-cj_q2 %>% 
  mutate(ci_lwr = xbar - crit*se,
         ci_upr = xbar + crit*se)
...
ggplot(SATE, aes(y=xbar, x= as.character(black))) + 
  geom_col() 
...
```{r}
### White result
2*pnorm(-abs(SATE$SATE[1]), mean=0, sd = SATE$SATE_se[1])
### Black result
2*pnorm(-abs(SATE$SATE[2]), mean=0, sd = SATE$SATE_se[2])
```
...
*Answer:* A type I error would mean that we reject the null hypothesis that criminal records have no effect when the truth is that they have no effect. We incorrectly state that there is an effect with a type I error. With a type II error, we incorrectly fail to reject the null, meaning we find support for no effect, when there is indeed an effect. The 0.05 critical value represents the likelihood of a type I error, meaning we've got a 5 percent chance of rejecting the null hypothesis, even it is true. We can decrease this risk by reducing the critical value.
